let RealEstate = artifacts.require("RealEstate");

contract("RealEstate", function(accounts){
    let realEstateInstance;

    it("컨트랙트의 소유자 초기화", function(){
    ...
    it("가나슈 두번째 계정으로 매물아이디 0번 매입 후, 이벤트 생성 및 매입자 정보와 buyers배열테스팅", function(){
        return RealEstate.deployed().then(function(instance){
            realEstateInstance = instance;
            ...
            assert.equal(receipt.logs[0].args._buyer, accounts[1], "매입자가 가나슈 두번째 게정이 아닙니다.");
